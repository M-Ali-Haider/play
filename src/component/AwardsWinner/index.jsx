...
export default function AwardsWinner({ isMobile }) {
  ...
  return (
    <div className={styles.awardsContainer}>
      <div className={styles.awardsInsideContainer}>
          ...
          <div className={styles.awardsGallery}>
            <div className={styles.awardsUnit}>
              <Image
                ...
                src={`${nextConfig.basePath}/images/${patterns[selectedUnit].first}`}
                width={500}
                height={500}
                alt="gallery-image"
              />
              <Image
                className={styles.awardsImage}
                src={`${nextConfig.basePath}/images/${patterns[selectedUnit].second}`}
                width={500}
                height={500}
                alt="gallery-image"
              />
            ...
            <div className={styles.awardsUnit}>
              <Image
                className={styles.awardsImage}
                src={`${nextConfig.basePath}/images/${patterns[selectedUnit].third}`}
                width={500}
                height={500}
                alt="gallery-image"
              />
              <Image
                className={styles.awardsImage}
                src={`${nextConfig.basePath}/images/${patterns[selectedUnit].fourth}`}
                width={500}
                height={500}
                alt="gallery-image"
              />
              <div className={styles.stats}>
                ...
                <p className={styles.gamingCollections}>
                  Gaming <span>Collections</span>
                </p>
              </div>
            </div>
          </div>
...
function NavBar({ selectedUnit, setSelectedUnit }) {
  ...
  return (
    <div className={styles.awardsNavbar}>
      {options.map((option, index) => {
        return (
          <div
            ...
            className={
              selectedUnit === index
                ? `${styles.awardsNavbarActivated} ${styles.awardsNavbarUnit}`
                : styles.awardsNavbarUnit
            }
            onClick={() => {
